...
- [x] **1.5.1:** AI tools keep the GUI responsive (edits, add_layer context scan, run_python Stop/timeout), Stop reaches network tools and never locks the chat, quitting mid-tool is safe; `.qmd`/`.qml` sidecars are refused as layers. Known limitations are listed in the release notes. Tag `strata-v1.5.1` uses the same packaging pipeline. Evidence: `docs/releases/strata-1.5.1.md`.
