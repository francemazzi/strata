...
import { platformAssets, platformPatterns, readJson, validateAcceptance, validateWindowsReport } from './release-integrity.mjs';
...
assertDraft(current);
for (const file of manifest.artifacts) {
  if (!current.assets.some(asset => asset.name === file.name && asset.digest === `sha256:${file.sha256}`)) throw new Error('Remote assets changed during acceptance.');
}
